          ...
          <div className="space-y-3">
            {followers.length > 0 ? (
              followers.map((follower: any) => {
                const isFollowing = followingStates[follower.id] ?? follower.isFollowing ?? false;
                const isOwnProfile = currentUser?.id === follower.id;
                
                return (
                  <Card key={follower.id} className="glass-card p-4 hover:bg-muted/20 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        <Link to={`/profile/${follower.id}`}>
                          <Avatar className="w-12 h-12">
                            ...
                            <AvatarFallback className="bg-primary text-primary-foreground font-semibold">
                              {follower.name?.slice(0, 2)?.toUpperCase() || follower.username?.slice(0, 2)?.toUpperCase()}
                            </AvatarFallback>
                          </Avatar>
                        </Link>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <Link to={`/profile/${follower.id}`} className="hover:underline">
                              <h3 className="font-semibold text-foreground truncate">{follower.name || follower.username}</h3>
                            </Link>
                            {follower.verified && (
                          ...
                          {follower.bio && (
                            <p className="text-sm text-foreground mt-1 line-clamp-2">{follower.bio}</p>
                          )}
                        </div>
                      </div>
